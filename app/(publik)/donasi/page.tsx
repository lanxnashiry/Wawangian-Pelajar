    ...
    <main>
      <section className="bg-[#102A43] px-5 py-16 text-white sm:px-8 sm:py-20 lg:px-10">
        <div className="mx-auto w-full max-w-7xl">
          ...
          <p className="mt-6 max-w-3xl text-base leading-8 text-[#D9E1E8]">Angka terkumpul tidak diketik bebas. Sistem menghitung 20% dari untung bersih rekap marketplace, lalu mengurangi hanya penyaluran yang memiliki bukti.</p>
          {ringkasan.sumberData === "contoh" ? <div role="status" className="mt-8 rounded-2xl border border-[#D1B779]/50 bg-[#D1B779]/10 p-5"><h2 className="text-xl font-black text-[#D1B779]">Simulasi transparansi · Data Contoh</h2><p className="mt-2 text-sm leading-6 text-slate-200">Seluruh nama, periode, jumlah, tujuan, dan visual bukti pada halaman ini hanya mengisi peninjauan MVP. Tidak ada klaim donasi atau penyaluran nyata.</p></div> : null}
      ...
      <section className="bg-white px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto w-full max-w-7xl">
          ...
          {penyaluran.length ? <div className="mt-7 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{penyaluran.map((item) => <article key={item.id} className="overflow-hidden rounded-3xl border border-[#DED3C2] bg-[#FAF7F1]">{item.bukti[0] ? <div className="relative aspect-[4/3] bg-white"><Image src={item.bukti[0]} alt={item.sumberData === "contoh" ? "Placeholder bukti penyaluran contoh" : `Bukti penyaluran kepada ${item.penerimaNama}`} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" unoptimized /></div> : null}<div className="p-5"><div className="flex flex-wrap gap-2 text-xs font-bold"><span className="rounded-full bg-[#E5F2EF] px-3 py-1 text-[#087477]">{labelPenerimaDonasi[item.penerimaJenis]}</span><span className="rounded-full bg-white px-3 py-1 text-[#4A4D52]">{formatTanggalDonasi(item.tanggal)}</span>{item.sumberData === "contoh" ? <span className="rounded-full bg-[#F6EACD] px-3 py-1 text-[#765B2B]">Data Contoh</span> : null}</div><h3 className="mt-4 text-xl font-black text-[#102A43]">{item.penerimaNama}</h3><p className="mt-2 text-2xl font-black text-[#C7A25A]">{formatRupiah(item.jumlah)}</p><p className="mt-3 line-clamp-3 text-sm leading-6 text-[#282B2F]">{item.tujuanDeskripsi}</p><Link href={`/donasi/${item.id}`} className="mt-5 inline-flex font-black text-[#087477] hover:underline">Periksa bukti →</Link></div></article>)}</div> : <div className="mt-7 rounded-3xl border border-dashed border-[#CFC3B2] bg-[#FAF7F1] p-9 text-center"><h3 className="text-xl font-black text-[#102A43]">Belum ada penyaluran terpublikasi</h3><p className="mt-2 text-sm leading-6 text-[#282B2F]">Entri tanpa bukti tetap menjadi draft dan tidak ditampilkan di sini.</p></div>}
        </div>
      </section>

...
export default async function EditProduk({ params, searchParams }: Properti) {
  ...
  const [{ data }, { data: daftarProfil }] = await Promise.all([
    supabase.from("produk").select("*,profil_rekomendasi(*)").eq("id", id).maybeSingle(),
    supabase.from("profil_rekomendasi").select("id,kode,nama").eq("aktif", true).order("nama"),
  ]);
  if (!data) notFound();
  const produk = petakanProduk(data as Parameters<typeof petakanProduk>[0]);
  return <main className="p-5 sm:p-8 lg:p-10"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black tracking-[0.16em] text-[#087477] uppercase">Produk</p><h1 className="mt-3 text-3xl font-black text-[#102A43]">Edit {produk.nama}</h1></div>{produk.aktif ? <form action={nonaktifkanProduk}><input type="hidden" name="id" value={id}/><button className="rounded-full border border-[#c0392b] px-4 py-2 text-sm font-black text-[#c0392b]">Nonaktifkan</button></form> : null}</div><div className="mt-7"><FormulirProduk produk={produk} pesan={pesan} daftarProfil={daftarProfil ?? []} /></div></main>;
}
